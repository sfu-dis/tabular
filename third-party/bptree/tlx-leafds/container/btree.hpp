...
public:
	...
	*/
    ...
    template <class F>
    void map_range(key_type start, key_type end, F f) {
        int cpuid = 0;
        ReaderWriterLock *parent_lock = nullptr;
        if constexpr(concurrent) {
            cpuid = sched_getcpu();
	...
	*/
    ...
    bool update(const value_type& value) const {
        auto key = key_of_value::get(value);
	    int cpuid = 0;
        ReaderWriterLock *parent_lock = nullptr;
        if constexpr(concurrent) {
            cpuid = sched_getcpu();
	...
	// leaf->slotdata.print();
	auto res = leaf->slotdata.update(value);
	if constexpr(concurrent) {
...
private:
    ...
    std::pair<result_t, bool> erase_one_descend(const key_type& key,
                               ...
                               ReaderWriterLock ** parent_lock, int cpu_id) {
        if (curr->is_leafnode())
        {
        ...
        else // !curr->is_leafnode()
        {
            ...
            if (slot == 0) {
                myleft =
                    ...
                    static_cast<InnerNode*>(left)->childid[left->get_slotuse() - 1];
    ...
    result_t merge_leaves(LeafNode* left, LeafNode* right,
        ...
        // TODO: leafDS merge api, puts everything from right into left
        left->slotdata.merge(&(right->slotdata));
