...
struct InlineBTreeWrapper : noname::index::GenericIndex {

  using BTreeType = noname::tabular::InlineBTree<Key, noname::table::OID>;

  explicit InlineBTreeWrapper(size_t key_size, noname::table::IndirectionArrayTable *table)
...
template<typename Key>
inline bool InlineBTreeWrapper<Key>::Insert(const char *key, size_t key_sz,
...
template<typename Key>
bool InlineBTreeWrapper<Key>::Search(const char *key,
...
template<typename Key>
inline int InlineBTreeWrapper<Key>::Scan(const char *start_key, bool start_key_inclusive,
                  const char *end_key, bool end_key_inclusive,
...
template<typename Key>
inline int InlineBTreeWrapper<Key>::ReverseScan(const char *start_key, bool start_key_inclusive,
                         const char *end_key, bool end_key_inclusive,
                         size_t key_sz, int scan_sz, char *&value_out) {
...
template<typename Key>
bool InlineBTreeWrapper<Key>::Update(const char *key, size_t key_sz
  , const char *value, size_t value_sz) {
  //static_assert(sizeof(Key) == key_sz);
...
template<typename Key>
inline bool InlineBTreeWrapper<Key>::Delete(const char *key, size_t key_sz) {
  return false;
  // throw duckdb::NotImplementedException("Delete not implemented yet");
}

/*
template struct InlineBTreeWrapper<GenericTPCCKey<4>>;
template struct InlineBTreeWrapper<GenericTPCCKey<8>>;
template struct InlineBTreeWrapper<GenericTPCCKey<12>>;
template struct InlineBTreeWrapper<GenericTPCCKey<16>>;
template struct InlineBTreeWrapper<GenericTPCCKey<40>>;
*/
