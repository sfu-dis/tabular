...
// but in a non-blocking fashion (i.e. through upgrade()).
...
struct BTreeOLC : public BTreeBase<Key, Value> {
  ...
  BTreeOLC() {
    std::cout << "========================================" << std::endl;
    std::cout << "BTree with original optimistic locks (upgrade)." << std::endl;
  ...
  bool insertOptimistically(const Key &k, Value v) {
    int restartCount = 0;
  restart:
...
#if 0
  bool insertPessimistically(const Key &k, Value v) {
    int restartCount = 0;
  restart:
    if (restartCount++) yield(restartCount);
  ...
  bool traverseToLeafEx(const Key &k, NodeBase *&node, uint64_t &versionNode) {
    int restartCount = 0;
  restart:
    if (restartCount++) yield(restartCount);
    bool needRestart = false;
