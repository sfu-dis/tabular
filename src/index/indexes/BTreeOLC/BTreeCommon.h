...
struct BTreeLeaf : public NodeBase {
  ...
  bool insert(const Key &k, Payload p) {
  ...
  bool update(const Key &k, Payload p) {
    assert(count <= maxEntries);
    if (count) {
      unsigned pos = lowerBound(k);
      if ((pos < count) && (data[pos].first == k)) {
  ...
  bool update(const Key &k, Payload p, bool opread) {
    assert(count <= maxEntries);
    if (count) {
      unsigned pos = lowerBound(k);
      if ((pos < count) && (data[pos].first == k)) {
...
struct BTreeBase {
  ...
  void makeRoot(const Key &k, NodeBase *leftChild, NodeBase *rightChild) {
  ...
  restart:
    ...
    while (leaf && count < range) {
      for (size_t i = pos; i < leaf->count && count < range; i++) {
        // -1 is implicitly converted to uint64_t max
        if ( end_key == nullptr || leaf->data[i].first <= *end_key ){
